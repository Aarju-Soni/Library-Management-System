...
function Borrow() {
  ...
  const getBookTitle = (id) =>
    books.find((book) => book.id === id)?.title || `Book #${id}`;
  ...
  const availableBooks = books.filter((book) => {
    const borrowed = records.some(
      (record) => record.book_id === book.id && !record.return_date,
    );
    ...
    <div className="brw-page">
      <div className="brw-layout">
        <section className="brw-ticket">
          ...
          <span className="brw-eyebrow">Circulation Desk</span>

          <h2 className="brw-title">New checkout</h2>

          {error && <p className="brw-error">{error}</p>}

          <form onSubmit={handleBorrow} className="brw-form">
            <div className="brw-field">
              <label className="brw-label">Book</label>
            ...
            <div className="brw-field">
              <label className="brw-label">Member</label>
        ...
        <section className="brw-log">
          <header className="brw-log-header">
            <h3 className="brw-log-title">Borrow records</h3>

            <span className="brw-log-count">{records.length} total</span>
          </header>
            ...
            <table className="brw-table">
              <thead>
                <tr>
                  ...
                  <th>Borrowed</th>
                  <th>Returned</th>
                  <th className="brw-th-actions">Actions</th>
                </tr>
              </thead>
                  ...
                  currentRecords.map((record) => {
                    ...
                    return (
                      <tr
                        key={record.id}
                        className={
                          active ? "brw-row brw-row-active" : "brw-row"
                        }
                      >
